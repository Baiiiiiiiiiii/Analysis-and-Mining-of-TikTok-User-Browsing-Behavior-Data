...
class Chart():
    ...
    def line_chart(self):
        ...
        chart = (
            ...
            .add_yaxis(
                '',
                [i[1] for i in self.data],  # 使用列表推导式从data中提取每个元组的第二个元素作为y轴的值
    ...
    def fl_chart(self):
        ...
        """
        chart = (
            Funnel(init_opts=opts.InitOpts(theme='light',width='600px',height='300px'))  # 使用Funnel类创建漏斗图实例
            .add('',self.data,label_opts=opts.LabelOpts(formatter='{d}%',position='right'))  # 添加数据，并设置数据标签显示为百分比
            .set_global_opts(
                title_opts=opts.TitleOpts(title=self.title),
                legend_opts=opts.LegendOpts(pos_left="0%",pos_top="55",orient='vertical')  # 图例在容器最左侧，距离顶部55像素点，并垂直排列
